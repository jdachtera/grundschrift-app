...
define("cordova/plugin/webos/camera", function(require, exports, module) {
    ...
    var imageToDataUrl = function(image, options) {
        var canvas = document.createElement("canvas"),
            context = canvas.getContext("2d"),
            exif2Angle = {6: 0, 3: 1, 8: 2, 1: 3},
            ratio = 1;

        if (options.targetWidth) {
            ratio = options.targetWidth / image.width;
        }
        if (options.targetHeight && (options.targetHeight / image.height) > ratio) {
            ratio = options.targetHeight / image.height;
        }

        var angle = exif2Angle[options.exifData.Orientation];

        var flip = (angle == 1 || angle == 3);

        canvas.width = ratio * (flip ? image.height: image.width);
        canvas.height = ratio * (flip ? image.width : image.height);

        context.translate(canvas.width / 2, canvas.height / 2);
        context.rotate(angle * Math.PI / 2);

        context.translate((flip ? -canvas.height: -canvas.width) / 2, (flip ? -canvas.width : -canvas.height) / 2);
    ...
    var imageUrlToDataUrl = function(imageUrl, options, successCallback, errorCallback) {
        var image = new Image();
        image.onload = function() {
            var dataUrl = imageToDataUrl(image, options);
    ...
    var getExifOrientation = function() {
        var switchObj = {
            'right': 1,
            'left': 3,
            'up': 6,
        ...
        if (window.PalmSystem) {
            return switchObj[PalmSystem.screenOrientation] || 1;

        }
        return 1;
    };

    var getPicture = function(successCallback, errorCallback, opts) {


        var k,
            getPicture,
            version = parseInt(device.version.split('.').shift()),
            options = {
                filename: '',
                ...
                flash: 0,
                reviewDuration: 1,
                exifData: {Orientation:0, Make:"HP", Model:"TouchPad"}
            },
        ...
        for (var i = 0; i < args.length && i < opts.length; i++) {
            options[args[i]] = opts[i];
        }

        getPicture = version < 3 ? getPictureV2 : getPictureV3;

        getPicture(function(file) {
            console.log(options, opts);
            if (options.destinationType == Camera.DestinationType.DATA_URL) {
                imageUrlToDataUrl(file, options, function(dataUrl) {
                    successCallback(dataUrl.replace(/^data:image\/(png|jpg);base64,/, ""));
                }, errorCallback);
            } else {
                successCallback(file);
            }
    ...
    var getPictureV2 = function(successCallback, errorCallback, options) {
    ...
    var getPictureV3 = function(successCallback, errorCallback, options) {
        var serviceLocation = "palm://com.palm.mediad/service",
            properties = {
                loaded: false,
                streaming: false
            },
        ...
        for (var i=0; i < body.childNodes.length;i++) {
            ...
            if (n.style.display !== 'none') {
                hiddenElements.push({element: n, display:n.style.display});
        ...
        video.setAttribute('style', 'position:absolute;z-index:100;background-color:black');


        service.Request(serviceLocation, {
            method: 'captureV3',
            onFailure: errorCallback,
            onSuccess: function(response) {
                serviceLocation = response.location;
                service.Request(serviceLocation, {
                    ...
                    onSuccess: function(response) {
                        ...
                        if (response.returnValue) {
                            ...
                            if (response.propertyValues && properties.loaded == false) {
                                properties.loaded = true;

                                for (i = 0; i < response.propertyValues.length; i++) {
                                    properties[response.propertyValues[i].name] = response.propertyValues[i].value;
                                }

                                device = properties.captureDevices[1];

                                format = {
                                    videoCaptureFormat: properties.supportedVideoFormats[0],
                                    imageCaptureFormat: properties.supportedImageFormats[0],
                                ...
                                service.Request(serviceLocation, {
                                    method: 'load',
                                    onFailure: errorCallback,
                                    parameters: {args: [device.deviceUri, format]}

                                });

                            }

                            if (response.propertyChange) {
                                properties[response.propertyChange.name] = response.propertyChange.value;

                            }

                            if (properties.streaming == false && properties.ready) {
                                properties.streaming = true;

                                video.src = serviceLocation;

                                video.play();
                                video.onclick = function() {
                                    options.exifData.Orientation = getExifOrientation();
                                    ...
                                    service.Request(serviceLocation, {
                                        ...
                                        parameters: {args: [options.filename, options]}
                                    });
                                };
                            }

                            if (properties.imagecapturecomplete) {
                                video.src = '';

                                successCallback(properties.lastImagePath);
                                service.Request(serviceLocation, {
                                    method: 'unload',
                                    parameters: {args: ['', {}]},
                                    onSuccess: function() {
                                ...
                                properties.imagecapturecomplete = 0;
                            }
                        }
                    }
                });
